...
static const char *string(const jx_value *o, const char *key) {
  const jx_value *v = jx_object_get(o, key);
  const char *s = jx_string_value(v);
  return s && strlen(s) == jx_string_length(v) && *s ? s : NULL;
}
static int quoted(asngn_buf *b, const jx_value *o, const char *key) {
  if (!string(o, key)) return -1;
  char *text = jx_write(jx_object_get(o, key), 0);
  int bad = !text || asngn_buf_appends(b, text) != ASNGN_OK;
  free(text); return bad;
}
static int field(asngn_buf *b, const jx_value *o, const char *key) {
...
static int strings_intact(const jx_value *v) {
  if (jx_typeof(v) == JX_STRING)
    return strlen(jx_string_value(v)) == jx_string_length(v);
  if (jx_typeof(v) == JX_ARRAY) {
    for (size_t i = 0; i < jx_array_len(v); i++)
      if (!strings_intact(jx_array_at(v,i))) return 0;
  } else if (jx_typeof(v) == JX_OBJECT) {
    for (size_t i = 0; i < jx_object_count(v); i++)
      if (!strings_intact(jx_object_value_at(v,i))) return 0;
  }
  return 1;
}
...
static int admitted(const jx_value *o, const char *schema) {
  jx_value *root = NULL;
  if (!schema || jx_parse(schema,strlen(schema),&root)) return 0;
  const jx_value *variants = jx_object_get(root,"oneOf");
  int ok = 0;
  for (size_t i = 0; !ok && i < jx_array_len(variants); i++) {
    const jx_value *props = jx_object_get(jx_array_at(variants,i),"properties");
    const char *action = string(o,"action");
    const char *allowed = string(jx_object_get(props,"action"),"const");
    if (!action || !allowed || strcmp(action,allowed)) continue;
    if (!strcmp(action,"call")) {
      const char *tool=string(o,"tool"), *expected=string(jx_object_get(props,"tool"),"const");
      if (!tool || !expected || strcmp(tool,expected)) continue;
      if ((jx_object_get(props,"path") != NULL) != (jx_object_get(o,"path") != NULL)) continue;
    } else if (!strcmp(action,"open")) {
      const jx_value *handles = jx_object_get(jx_object_get(props,"input"),"enum");
      const char *input = string(o,"input");
      for (size_t j = 0; input && j < jx_array_len(handles); j++) {
        const char *h = jx_string_value(jx_array_at(handles,j));
...
static int step(asngn_buf *b, const jx_value *o) {
  const char *action = string(o, "action");
  size_t fields = 1;
  if (!action) return -1;
  ...
  if (input) {
    ...
    if (call) {
      ...
      const jx_value *args = jx_object_get(o, "arguments");
      ...
      if (path && !strcmp(tool, "fs.write") && !args) {
        ...
        if (path || jx_typeof(args) != JX_OBJECT || !strings_intact(args)) return -1;
        char *text = jx_write(args, 0);
        int bad = !text || asngn_buf_appends(b, text) != ASNGN_OK;
  ...
  return jx_object_count(o) != fields || asngn_buf_appends(b, "}\n") != ASNGN_OK;
}
...
asngn_err asngn_protocol_decode(asngn_task_kind kind, const char *schema, char **text) {
  ...
  asngn_buf b;
  int bad = !text || !*text;
  if (bad || jx_parse(*text, strlen(*text), &o)) return ASNGN_ERR_PROTOCOL;
  asngn_buf_init(&b);
  if (jx_typeof(o) != JX_OBJECT) bad = 1;
    ...
    const jx_value *score = jx_object_get(o, "score");
    const char *critique = string(o, "critique");
    long long n = jx_int_value(score);
    bad = !jx_is_int(score) || n < 0 || n > 10 || !critique ||
        jx_object_count(o) != 2 || strlen(critique) > ASNGN_STEP_META_MAX ||
        strpbrk(critique, "|\r\n") != NULL;
