...
asngn_err mcp_job_poll(mcp_job *j, unsigned long long cursor, asmodel_json_value **out) {
  asmodel_json_value *o = asmodel_json_object(), *events = asmodel_json_array();
  int ok = o && events;
  *out = NULL;
  if (!j->done) {
  ...
  if (cursor > j->next) { os_mutex_unlock(&j->mu); asmodel_json_free(o); asmodel_json_free(events); return ASNGN_ERR_INVALID; }
  ok &= asmodel_json_object_set(o, "cursor_gap", asmodel_json_bool(cursor < first)) == 0;
  ...
  for (; ok && cursor < j->next; cursor++) {
    ...
    ok = v != NULL;
    ok &= asmodel_json_object_set(v, "cursor", asmodel_json_int((long long)cursor)) == 0;
    ok &= asmodel_json_object_set(v, "kind", asmodel_json_int(j->events[i].kind)) == 0;
    ok &= asmodel_json_object_set(v, "text", asmodel_json_string(j->events[i].text ? j->events[i].text : "")) == 0;
    ok &= asmodel_json_object_set(v, "truncated", asmodel_json_bool(j->events[i].truncated)) == 0;
    ok &= asmodel_json_array_push(events, v) == 0;
  }
  ok &= asmodel_json_object_set(o, "next_cursor", asmodel_json_int((long long)j->next)) == 0;
  os_mutex_unlock(&j->mu);
  ok &= asmodel_json_object_set(o, "events", events) == 0;
  ok &= asmodel_json_object_set(o, "done", asmodel_json_bool(j->done)) == 0;
  ok &= asmodel_json_object_set(o, "task_id", asmodel_json_string(mcp_job_id(j))) == 0;
  if (j->done) {
    ok &= asmodel_json_object_set(o, "outcome", asmodel_json_string(asngn_err_name(j->outcome))) == 0;
    ok &= asmodel_json_object_set(o, "answer", asmodel_json_string(j->result.answer ? j->result.answer : "")) == 0;
    ok &= mcp_work_status(j->session,asngn_task_work_revision(j->task),o) == ASNGN_OK;
  }
  if (!ok) { asmodel_json_free(o); return ASNGN_ERR_NOMEM; }
  *out = o;
  return ASNGN_OK;
}
